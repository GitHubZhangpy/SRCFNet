...
base_target_size = base_target_sz;  % use for updating
...
% new rhs sample 
xyf_vec = bsxfun(@times, yf_vec, conj(xlf_reshaped));
xyf = bsxfun(@times, yf, conj(xlf));
xxf = bsxfun(@times,xlf,conj(xlf));
xy_dfs = real(dfs_matrix * double(xyf_vec));
new_hf_rhs = xy_dfs(:);
%%new_hf_rhs=b_t
...
switch params.init_strategy
    case 'const_reg'       % exact solution for constant regularization
        hf_init = bsxfun(@rdivide, xyf, hf_init_autocorr + params.reg_window_min^2);
...
% rect_position(frame,:) = [pos([2,1]) - floor(target_sz([2,1])), target_sz([2,1])*2];
%visualization
if visualization == 1
    rect_position_vis = [pos([2,1]) - target_sz([2,1])/2, target_sz([2,1])];
%     rect_position_vis = [pos([2,1]) - target_sz([2,1]), target_sz([2,1])*2];
...
for frame = 2:num_frames,
    ...
    if frame > 1
        ...
        while iter <= refinement_iterations && any(old_pos ~= pos)
            ...
            % response
%             response = real(ifft2(ifftshift(ifftshift(responsef_padded,2),1)));
    ...
    % take the DFT and vectorize each feature dimension
    xlf = fft2(xl);
    xyf = eta* bsxfun(@times, yf, conj(xlf))+(1-eta)*xyf;
    xxf=eta*bsxfun(@times,xlf,conj(xlf))+(1-eta)*xxf;
   [hf] = net_update(xxf,xyf,hf,use_sz,params,base_target_size);
    % debug visualization
    if debug
        % plot filters
        figure(20)
        ...
        for disp_layer = 1:feature_dim
            subplot(subplot_rows,subplot_cols,disp_layer);
            imagesc(ifft2(conj(filter(:,:,disp_layer)), 'symmetric'));
            colorbar;
            axis image;
        end
...
fps_temp = numel(s_frames)/time;

results.type = 'rect';
results.res = rect_position;
results.fps = fps_temp;
